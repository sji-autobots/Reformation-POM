...
public class CartPage {
        ...
        // Locators
        private final By firstProduct = By.xpath("(//a[contains(@href,'product')])[1]");

        private final By sizeButtons = By.xpath("//button[@data-attr='size' and @data-attr-selectable='true']");

        private final By addToCartBtn = By.xpath("//button[contains(.,'Add')]");

        private final By cartIcon = By.xpath("//a[contains(@href,'cart')]");

        // Remove flow locators (based on your HTML)
        private final By removeIcon = By.xpath("(//button[@data-line-item-component='remove-action'])[1]");

        private final By confirmRemoveButton = By
                        .xpath("(//button[@data-line-item-component='remove-confirm-action'])[1]");

        private final By emptyCartMessage = By.xpath("//*[contains(text(),'Your cart is empty')]");
        ...
        // Get pickup label text
        public String getPickupText() {
                // Locate the pickup label element and return its text
                // Adjust the locator based on your actual HTML structure
                WebElement pickupLabel = driver.findElement(
                                By.cssSelector(".product-line-item__info"));
                return pickupLabel.getText();
        }
        ...
        public boolean isCartEmpty() {
                ...
                try {
                        return wait.until(
                                        ExpectedConditions.visibilityOfElementLocated(emptyCartMessage)).isDisplayed();
                } catch (Exception e) {
                        return false;
                }
        }
