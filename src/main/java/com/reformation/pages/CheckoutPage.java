...
public class CheckoutPage {
    ...
    public void enterCreditCardNumber(String cardNumber) {
        ...
        ((org.openqa.selenium.JavascriptExecutor) driver)
                .executeScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", cardNumberInput);
        cardNumberInput.click();
        // Switch back to main context to click the radio label
        driver.switchTo().defaultContent();
        WebElement radioLabel = cardWait
                .until(ExpectedConditions.elementToBeClickable(By.xpath("//label[@id='checkout-stripe-radio-label']")));
        ((org.openqa.selenium.JavascriptExecutor) driver)
                .executeScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", radioLabel);
        radioLabel.click();
    ...
    public void fillShippingAddress(String firstName, String lastName, String phone, String address) {
        ...
        WebElement firstNameInput = shippingWait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shippingFirstNamedefault']")));
        firstNameInput.clear();
        firstNameInput.sendKeys(firstName);
        WebElement lastNameInput = shippingWait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shippingLastNamedefault']")));
        lastNameInput.clear();
        lastNameInput.sendKeys(lastName);
        WebElement phoneInput = shippingWait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shippingPhoneNumberdefault']")));
        phoneInput.clear();
        phoneInput.sendKeys(phone);

        WebElement addressInput = shippingWait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shippingAddressOnedefault']")));
        addressInput.click();
        addressInput.clear();
        ...
        WebElement cityInput = shippingWait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shippingAddressCitydefault']")));
        cityInput.clear();
        cityInput.sendKeys("Pinckneyville");

        // Fill state
        WebElement stateSelect = shippingWait
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//select[@id='shippingStatedefault']")));
        org.openqa.selenium.support.ui.Select stateDropdown = new org.openqa.selenium.support.ui.Select(stateSelect);
        stateDropdown.selectByVisibleText("Illinois");

        // Fill zip code
        WebElement zipInput = shippingWait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shippingZipCodedefault']")));
        zipInput.clear();
    ...
    public void verifyAccountSignupSection() {
        ...
        WebElement passwordField = accountSection
                .findElement(By.xpath(".//input[@type='password' and @name='dwfrm_newPasswords_newpassword']"));
        WebElement confirmPasswordField = accountSection
                .findElement(By.xpath(".//input[@type='password' and @name='dwfrm_newPasswords_newpasswordconfirm']"));
    ...
    public void clickPaymentButtonWithRetry() {
        ...
        WebElement paymentButton = wait
                .until(ExpectedConditions.elementToBeClickable(By.xpath("//button[@id='shipping-page-button']")));
        ...
        try {
            ...
            WebElement errorModal = shortWait.until(
                    ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div.address-verification__modal")));
            ...
            if (errorModal.isDisplayed()
                    && addressErrorMessage.getText().contains("Your shipping address looks invalid")) {
    ...
    public void verifyDeliveryAddressContains(String expected) {
        ...
        WebElement deliveryAddressField = wait.until(
                ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".checkout__card.shipping-summary")));
        String address = deliveryAddressField.getText();
        if (!address.contains(expected)) {
            throw new AssertionError(
                    "Delivery address does not contain '" + expected + "' as expected. Actual: " + address);
        }
    }

    public void verifyCardSelectedContains(String cardType) {
        WebDriverWait wait = new WebDriverWait(driver, java.time.Duration.ofSeconds(10));
        WebElement savedCardDropdown = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("stripe-card")));
        org.openqa.selenium.support.ui.Select select = new org.openqa.selenium.support.ui.Select(savedCardDropdown);
    ...
    public void proceedToReview() {
        ...
        WebElement reviewButton = wait
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//button[@id='payment-page-button']")));
        reviewButton.click();
    }
    ...
    public void placeOrder() {
        ...
        try {
            ...
            ((org.openqa.selenium.JavascriptExecutor) driver).executeScript("arguments[0].click();", btnPay);
            System.out.println("[DEBUG] JavaScript click succeeded for btnPay CTA.");
        } catch (Exception e) {
            System.out.println(
                    "[ERROR] Could not switch to Intrnl_CO_Container iframe or click btnPay CTA: " + e.getMessage());
