...
emailField.click();
emailField.clear();
emailField.sendKeys("refqatester@gmail.com");
...
passwordField.click();
passwordField.clear();
passwordField.sendKeys("Web_testing123");
...
WebElement showBenefitsBtn = wait.until(
        ExpectedConditions.presenceOfElementLocated(
                By.cssSelector("span.account__fwb-toggle__trigger")
        )
);

((JavascriptExecutor) driver).executeScript(
        "arguments[0].scrollIntoView({block:'center'});", showBenefitsBtn
);
Thread.sleep(500);

((JavascriptExecutor) driver).executeScript("arguments[0].click();", showBenefitsBtn);
System.out.println("Show Benefits clicked");
