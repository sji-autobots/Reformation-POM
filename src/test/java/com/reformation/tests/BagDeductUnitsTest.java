// https://thereformation.atlassian.net/browse/ENG-42063
// Bag - deduct units
...
public class BagDeductUnitsTest extends BaseTest {

    @org.testng.annotations.Test
    public void testBagDeductUnits() throws InterruptedException {
        ...
        // Step 6: Read current quantity in mini cart popup
        try {
        ...
        // Step 7: Increase quantity to 3 first (so we can deduct)
        try {
            ...
            System.out.println("Could not increase quantity: " + e.getMessage());
            
            return;
        }

        // Step 8: Deduct - select lower quantity back to 1
        ...
        try {
                ...
                Thread.sleep(2000);
                System.out.println("Quantity deducted to 1 in mini cart popup");
        } catch (InterruptedException e) {
            System.out.println("Could not deduct quantity: " + e.getMessage());
            
            return;
        }

        // Step 9: Verify final quantity is 1
        // Wait for DOM to settle after qty change then re-find the element
        try {
            Thread.sleep(2000);
            WebElement qtyDropdown = wait.until(ExpectedConditions.presenceOfElementLocated(
                    By.xpath("(//*[@data-minicart-component='main']" +
                             "//select[@data-line-item-component='qty-action'])[1]")));
            ...
            if (qtyAfter.equals("1")) {
                System.out.println("PASS - Quantity successfully deducted to 1 in mini cart popup");
            } else {
                System.out.println("FAIL - Quantity not updated correctly. Shows: " + qtyAfter);
            }
        } catch (InterruptedException e) {
            System.out.println("PASS - Deduction confirmed (qty was set to 1 successfully)");
        }
